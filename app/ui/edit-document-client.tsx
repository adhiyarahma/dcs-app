...
function EditModeSelector({
  ...
  const nextRevision =
    document.revision >= MAX_REVISION ? 0 : document.revision + 1;
  return (
    <div className="space-y-4">
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mb-6">
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          {document.doc_number} · Rev{" "}
          {String(document.revision).padStart(2, "0")} · {document.type_name}
        </p>
      </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5 leading-relaxed">
              Salah ketik, typo, atau kesalahan input. Nomor revisi tidak
              berubah.
            </p>
            <p className="text-xs text-blue-600 font-medium mt-2">
              Rev {String(document.revision).padStart(2, "0")} → tetap Rev{" "}
      ...
      >
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5 leading-relaxed">
              Dokumen berubah substansial. Versi lama jadi{" "}
              <span className="font-medium">kadaluarsa</span>, versi baru
              dibuat.
            </p>
            <p className="text-xs text-amber-600 font-medium mt-2">
              Rev {String(document.revision).padStart(2, "0")} → kadaluarsa ·
              Baru: Rev {String(nextRevision).padStart(2, "0")}
            </p>
          </div>
        </div>
      </button>
...
function EditForm({
  ...
  const fieldConfig = getFieldConfig(
    ...
    selectedType?.name ?? document.type_name
  );
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (mode === "koreksi" && formData.get("status") === "dihapus") {
      const confirmed = window.confirm(
        'Yakin ingin mengubah status dokumen ini menjadi "Dihapus"?'
      );
      if (!confirmed) return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      >
        ...
        <p className="text-sm mt-2 text-slate-600">
          {isRevisi ? (
            <>
              Dokumen lama (Rev {String(document.revision).padStart(2, "0")})
              akan jadi <strong>kadaluarsa</strong>. Baris baru: Rev{" "}
              <strong>{String(newRevision).padStart(2, "0")}</strong>.
            </>
          ) : (
            <>
              Perubahan langsung disimpan. Rev{" "}
              <strong>{String(document.revision).padStart(2, "0")}</strong>{" "}
              tidak berubah.
            </>
          )}
        </p>
        ...
        {fieldConfig.showExpiryDate && (
          <div>
            ...
            <input
              ...
              required={fieldConfig.showExpiryDate}
              className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        )}
